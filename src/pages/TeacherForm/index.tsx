...
const TeacherForm = () => {
  ...
  const setScheduleItemValue = (position: number, field: 'week_day' | 'from' | 'to', value: string) => {
     setScheduleItems(oldItems => oldItems.map((item, index) => 
      index === position
        ? { ...item, [field]: value }
        : item
    ))
  ...
  const handlerCreateClass = (e: FormEvent) => {
    e.preventDefault()
